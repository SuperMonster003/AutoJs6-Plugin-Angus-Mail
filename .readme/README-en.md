...
Version 1.1.0 adds background watches (roadmap P8): the Watches page, the foreground service and the "On mail arrived" task of AutoJs6; every item of phases P0 to P7 shipped with 1.0.0 and 1.0.1, with evidence in [ROADMAP.md](https://github.com/SuperMonster003/AutoJs6-Plugin-Angus-Mail/blob/master/ROADMAP.md). Requires AutoJs6 6.8.0 (build 5282) or later; the "On mail arrived" task needs the host build with mail contract version 2; the full script API reference is in the [AutoJs6 documentation](https://docs.autojs6.com/#/mail).
...
- `Fix` Gmail and Outlook.com columns of the provider matrix (mail roadmap P6, backfilled once the maintainer renewed the Gmail token and provided three Outlook.com / Hotmail accounts): Gmail passed session test, folders, send, listing, server search, body, attachment, flags and keyword, folder creation, move, watch and POP3 with the token, its IDLE really pushes (new mail is reported about 30 s after delivery, the cadence Gmail notifies at), and the P5 device matrix (baseline, plugin killed, Wi-Fi off) was repeated on a Redmi (API 33) on the IDLE path (`docs/dev/p6-provider-matrix.md`, `docs/dev/p5-watch-evidence.md`). Two findings changed the mail core: Gmail advertises `UTF8=ACCEPT`, Angus Mail enabled it and Gmail then rejected every search with non-ASCII text (`BAD Could not parse command`), so a Chinese subject search fell back to the client; the extension is no longer enabled and the search carries `CHARSET UTF-8` as on every other server (`Utf8SearchTest`). The client-side filter now reports its progress to the debug trace every 25 candidates, because a body search with `fallback: 'always'` that finds fewer than `limit` matches scans the whole 2000-candidate window at two or three round trips per candidate, about 43 minutes on Gmail from this network. Outlook.com: all three accounts answer `Basic authentication is disabled` on IMAP and POP3 and `535` on SMTP to an app password, so the preset's XOAUTH2-only rule stands and the operation rows wait for a token (roadmap P9); with the preset bypassed the server's own refusal (`LOGINDISABLED`, only `AUTH=XOAUTH2`) is reported as `AUTH_MECHANISM_UNSUPPORTED` instead of `SERVER_ERROR` (`LoginDisabledTest`)
...
- `Improvement` Release size (mail roadmap P6): the R8 rules no longer keep the whole `jakarta.mail`, `jakarta.activation` and Angus Mail namespaces by name, only the 20 classes the libraries load by name (the IMAP / POP3 / SMTP stores and transports, their providers, the stream provider, the activation registries and the five data content handlers), each with the resource that names it recorded in `app/proguard-rules.pro` and `docs/dev/p6-size.md`. The universal release APK shrinks from 2,254,035 to 2,153,957 bytes (download size 1,507,931 to 1,407,295), 2,319 DEX methods and 162 library classes go (the `MailHandler` logging component, the GraalVM feature, the image handlers, the SASL clients); the instrumentation suite passes against the shrunk build on a device (27 tests)
