...
- `개선` 릴리스 빌드에도 관리자의 Google OAuth 2.0 클라이언트 id (2026-09-22 에 등록한 Android 클라이언트) 가 담겨 Gmail 프리셋에서 "Google 로 로그인 (브라우저)" 를 제공합니다. Google 프로젝트는 테스트 상태이므로 프로젝트의 테스트 사용자만 로그인할 수 있고 그 갱신 토큰은 7 일 후 만료됩니다 (이후 "다시 로그인" 을 표시합니다); 다른 Google 계정에는 Google 의 접근 거부 페이지가 표시되며, 토큰과 앱 비밀번호 경로는 그대로입니다.
