...
버전 1.1.0 은 백그라운드 감시 (로드맵 P8) 를 추가합니다: 감시 페이지, 포그라운드 서비스, AutoJs6 의 "메일 도착 시" 작업. 단계 P0 부터 P7 까지의 모든 항목은 1.0.0 과 1.0.1 로 출시되었으며 증거는 [ROADMAP.md](https://github.com/SuperMonster003/AutoJs6-Plugin-Angus-Mail/blob/master/ROADMAP.md) 에 있습니다. AutoJs6 6.8.0 (빌드 5282) 이상이 필요하고, "메일 도착 시" 작업에는 메일 계약 버전 2 를 포함한 호스트 빌드가 필요합니다. 스크립트 API 의 전체 참조는 [AutoJs6 문서](https://docs.autojs6.com/#/mail) 에 있습니다.
...
- `수정` 공급자 매트릭스 (메일 로드맵 P6): QQ, 163, 126, yeah.net, Sina 를 실제 계정으로 발송, 목록, 서버와 클라이언트의 중국어 검색, 본문, 첨부 바이트, 플래그, 폴더 생성, 이동, 감시, POP3 까지 실행하고 (`ProviderMatrixProbe`, `.python/run_provider_matrix.py`, `docs/dev/p6-provider-matrix.md`) 차이를 프리셋 notes 에 기록함: QQ 는 중국어 SEARCH 에 OK 와 빈 결과로 답하고 (`fallback: 'always'` 사용), Message-ID 와 ENVELOPE 의 To 표시 이름을 바꾸며, CREATE 를 거부하고, 스스로 저장한 보낸 편지 사본은 한동안 UID 로 지정할 수 없음. 163 은 최근 메일에 대한 텍스트 검색에 결과를 주지 않음. Sina 는 ALL, SINCE 와 플래그 검색 키만 받음. 다섯 곳 모두 사용자 키워드를 저장하지 않음. 클라이언트 결함도 수정함: SASL continuation 으로 거부된 POP3 XOAUTH2 로그인 (Gmail) 은 Angus Mail 이 거부를 무시하여 재시도 가능한 `IO_FAILED` 로 보고되었으나, 이제 메일 코어가 로그인을 검증하고 `AUTH_FAILED` 를 반환함 (`Pop3OAuthScriptedTest`). Gmail (토큰 만료), Outlook.com 과 iCloud (계정 없음) 는 미검증 상태로 남음
- `수정` 수명 주기 매트릭스 (메일 로드맵 P6): 세션과 watch 를 유지하는 스크립트를 Redmi (API 33) 에서 여덟 가지 방식으로 종료: 정상 종료, watch 와 클라이언트를 연 채 `exit()`, `engines.stopAll()`, 호스트 강제 종료, 플러그인 강제 종료, 플러그인 제자리 업그레이드, 플러그인 비활성화, 플러그인 제거; 모든 경우 플러그인의 연결, 바인딩, 파일 디스크립터가 30 s 안에 스크립트 이전 상태로 돌아왔고 호스트는 서버에 직접 연결하지 않음 (`docs/dev/p6-lifecycle-matrix.md`, `.python/run_lifecycle_matrix.py`, `docs/smoke/lifecycle.js`). 매트릭스가 누수 하나를 찾아 수정: Angus Mail 은 쓰기 시간 제한을 위해 소켓마다 스레드 풀을 만들고 TLS 소켓이 그 래퍼를 통해 닫힐 때만 종료하는데, 아래의 평문 소켓이 cancel 이나 watch 중지로 먼저 닫히면 Android 가 그 단계를 건너뛰어 그런 연결마다 프로세스 수명 동안 스레드 하나가 남았음; 메일 코어는 이제 공유 데몬 타이머 하나를 Angus 에 넘김 (`WriteTimeouts`, `WriteTimeoutExecutorTest`)
- `수정` 공급자 매트릭스의 Gmail 및 Outlook.com 열 (메일 로드맵 P6, 유지 관리자가 Gmail 토큰을 갱신하고 Outlook.com / Hotmail 계정 세 개를 제공한 뒤 보충): Gmail 은 토큰으로 세션 테스트, 폴더, 보내기, 목록, 서버 검색, 본문, 첨부, 플래그와 키워드, 폴더 생성, 이동, watch, POP3 를 통과했고 IDLE 이 실제로 푸시하며 (새 메일은 배달 후 약 30 s 에 보고되는데 이것이 Gmail 의 알림 주기), P5 기기 매트릭스 (기준, 플러그인 강제 종료, Wi-Fi 끄기) 를 Redmi (API 33) 에서 IDLE 경로로 다시 실행 (`docs/dev/p6-provider-matrix.md`, `docs/dev/p5-watch-evidence.md`). 두 가지 발견이 메일 코어를 바꿨다: Gmail 은 `UTF8=ACCEPT` 를 광고하고 Angus Mail 이 이를 켜면 Gmail 은 비 ASCII 텍스트가 든 모든 검색을 거부하므로 (`BAD Could not parse command`) 한글이나 중국어 제목 검색이 클라이언트로 폴백했다; 이제 이 확장을 켜지 않고 검색은 다른 서버에서처럼 `CHARSET UTF-8` 을 붙인다 (`Utf8SearchTest`). 클라이언트 측 필터는 이제 후보 25 개마다 디버그 추적에 진행 상황을 보고한다. `fallback: 'always'` 본문 검색은 일치가 `limit` 에 못 미치면 2000 후보 창 전체를 훑고 후보마다 두세 번 왕복하여 이 네트워크에서 Gmail 은 약 43 분이 걸리기 때문. Outlook.com: 세 계정 모두 앱 비밀번호에 IMAP 과 POP3 에서 `Basic authentication is disabled`, SMTP 에서 `535` 로 답하므로 프리셋의 XOAUTH2 전용 규칙은 유효하고 작업 행은 토큰을 기다림 (로드맵 P9); 프리셋을 우회하면 서버 자체의 거부 (`LOGINDISABLED`, `AUTH=XOAUTH2` 만) 는 `SERVER_ERROR` 대신 `AUTH_MECHANISM_UNSUPPORTED` 로 보고된다 (`LoginDisabledTest`)
...
- `개선` 성능 기준선 (메일 로드맵 P6): 시드된 로컬 서버 (10000통 받은편지함, 50 MiB 첨부) 를 대상으로 목록, 검색, 다운로드, 발송, 1시간 IDLE 대기를 JVM, Redmi (API 33), Sony (API 28) 에서 측정하고 수치를 `docs/dev/p6-performance-baseline.md` 에 기록. 이로 인해 두 가지를 변경: IMAP 페치 크기를 64 KiB 에서 1 MiB 로 올려 50 MiB 첨부의 왕복이 약 1100회에서 69회로 (루프백 10 MiB/s, 다운로드 중 메모리는 여전히 약 2 MiB); 클라이언트 측 검색은 `limit` 건 일치 시 중단하고 모든 후보를 훑지 않음. 대기에서 플러그인은 약 32 MiB PSS, 시간당 CPU 3.5 s 와 트래픽 40 KB 이며, 장시간 감시에는 호스트의 포그라운드 서비스가 필요 (없으면 Android 9 가 31분에 호스트와 플러그인을 캐시된 빈 프로세스로 종료)
- `개선` 릴리스 크기 (메일 로드맵 P6): R8 규칙은 더 이상 `jakarta.mail`, `jakarta.activation`, Angus Mail 네임스페이스 전체를 이름 그대로 유지하지 않고, 라이브러리가 이름으로 로드하는 20개 클래스 (IMAP / POP3 / SMTP 스토어와 트랜스포트, 해당 Provider, 스트림 제공자, activation 레지스트리, 데이터 콘텐츠 핸들러 5개) 만 유지하며 각 규칙의 근거 리소스를 `app/proguard-rules.pro` 와 `docs/dev/p6-size.md` 에 기록. universal 릴리스 APK 는 2,254,035 바이트에서 2,153,957 바이트로 (다운로드 크기 1,507,931 에서 1,407,295), DEX 메서드 2,319개와 라이브러리 클래스 162개 제거 (`MailHandler` 로깅 구성 요소, GraalVM 기능, 이미지 핸들러, SASL 클라이언트); 계측 테스트 스위트가 실기기에서 축소된 빌드에 대해 통과 (27건)
