...
- [ ] (维护者) 注册 OAuth 客户端: Google (`https://mail.google.com/` scope, Android 客户端类型绑定包名与签名指纹, 发布前需通过敏感 scope 审核) 与 Microsoft Entra (公共客户端, 移动重定向 URI, scope `https://outlook.office.com/IMAP.AccessAsUser.All` / `POP.AccessAsUser.All` / `SMTP.Send` / `offline_access`); 客户端 ID 以 `resValue` 注入, 不含密钥 2026-09-21: Microsoft Entra 公共客户端已由维护者注册 (个人账户, 重定向 `http://localhost`, 允许公共客户端流), `.python/outlook_oauth_login.py` 以其在 PC 上取得令牌供 P6 矩阵使用; Google 客户端与插件内置流程仍待办.
