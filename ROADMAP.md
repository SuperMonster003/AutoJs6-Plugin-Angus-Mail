...
| D41 | 不启用 `UTF8=ACCEPT` | 服务商矩阵 (P6, Gmail): Gmail 广告 `ENABLE UTF8=ACCEPT`, Angus Mail 对广告该扩展的服务器一律启用, 之后按 RFC 6855 以不带 `CHARSET` 的 UTF-8 quoted string / literal 发送搜索文本, Gmail 对此答 `BAD Could not parse command` (真实账户 2026-09-20), 而 `SEARCH CHARSET UTF-8` + literal 命中; 其余预设服务商都不广告该扩展. `IdentifyingImapProtocol.enable` 跳过它: 搜索总带 `CHARSET UTF-8`, 文件夹名总用 modified UTF-7, 即所有服务器都走的路径 (`Utf8SearchTest`) |
...
| D42 | 后台守望前台服务用 `specialUse` 而非 `dataSync` | P8 落地时核实: Android 15 起 `dataSync` 类型的前台服务每 24 小时累计只能运行 6 小时且不能由 `BOOT_COMPLETED` 启动, 而守望是等待推送的长连接; 改用 `specialUse` (子类型 `mail_background_watch`, `PROPERTY_SPECIAL_USE_FGS_SUBTYPE` 属性), 权限 `FOREGROUND_SERVICE_SPECIAL_USE`; 通知渠道低优先级. 侧载插件不受应用商店对 `specialUse` 的申报流程约束. (P8) |
| D44 | 浏览器登录的令牌只在插件内解密, 会话拿到的是访问令牌 | P9 落地时定案: `SecretKind.OAUTH2` 记录存整份令牌文档 (访问 / 刷新令牌与过期时刻, AES-GCM 加密), 账户文档只带 `oauth` 元数据 (`provider` / `authorizedAt` / `expiresAt` / `needsReauth`); 别名会话 (脚本, 连接测试, 后台守望) 一律经 `AccountSecrets.withUsableSecret` 取访问令牌, 剩余不足 5 分钟先刷新 (按账户串行), 刷新被拒 (`invalid_grant`) 标记 `needsReauth` 并以 `AUTH_FAILED` 失败; 令牌从不进 Binder 字段, `mail.accounts.list()` 只报告 `oauth` 元数据, `auth` 保持 `xoauth2` (不新增 `oauth2` 枚举值, 声明与文档只加可选 `oauth` 对象) |
| D45 | 重定向用自定义 scheme, 客户端 id 由构建注入 | Microsoft 用 `<applicationId>://oauth2/microsoft` (Entra "移动和桌面应用程序" 平台), Google 用反转客户端 id scheme `com.googleusercontent.apps.<id>:/oauth2redirect` (Android 客户端类型); 不用 `https` App Link (无需托管 `assetlinks.json`, 且 Entra 个人账户对 App Link 无额外收益); 客户端 id 与租户从 Git 忽略的 `oauth-clients.properties` 在构建时注入 `resValue` 与 manifest 占位符 `oauthGoogleScheme`, 缺失时该服务商的登录项在认证对话框中禁用并说明 |
| D43 | 宿主任务走唤醒广播, `openTrigger` 留给脚本级 API | 宿主的 "邮件到达" 任务在宿主运行与否两种状态下都由插件的显式 `MAIL_TRIGGER` 广播 (PLUGIN 权限, Manifest 接收器) 触发, 宿主不为任务保持 `openTrigger` 订阅 (免去宿主常驻绑定与订阅生命周期); 去重与节流在宿主 `MailTriggerDispatcher` 内完成. 用户 "强行停止" 宿主后 Android 不向停止状态的包投递广播, 这是系统规则, 不做规避. `openTrigger` / `IMailTrigger` 保留在契约中, 供后续脚本级 `mail.triggers` API 使用. (P8) |
...
- [x] (测试) 服务商兼容矩阵 (`docs/dev/p6-provider-matrix.md`): QQ / 163 / 126 / Gmail (App Password) / Outlook.com (XOAUTH2 令牌, 可用时) / iCloud (App Password) 各跑 发信 / 列表 / 中文搜索 (服务器与客户端回退) / 正文 / 附件 / 标记 / 移动 / IDLE; 记录每家的特殊行为 (163 `ID` 命令, QQ 的已发送文件夹名, Gmail 的 `[Gmail]/` 前缀与标签语义, iCloud 的文件夹层级分隔符) 并回填附录 C. (Yahoo / Aliyun 不进矩阵: 维护者 2026-09-19 确认无法提供有效账户, 两家预设在附录 C 与 `providers.json` notes 标注为未核实.) (2026-09-19 完成, build 41: `ProviderMatrixProbe` (mail-core, 由 `.python/run_provider_matrix.py` 驱动, 无 `build/p6/matrix.properties` 时跳过, 输出只含 `***@domain` 与脱敏协议轨迹) 对 QQ / 163 / 126 / yeah.net / Sina 各跑 test / folders / 发信 (中文主题, 正文, 显示名, 文件名) / 列表 / 服务器与客户端中文搜索 / 正文 / 附件字节 / 标记 / 建夹与移动 / watch (auto) / POP3 / 清理; 五家全部通过, 差异记入附录 C 与 `providers.json` notes (QQ: 非 ASCII SEARCH 答 OK 零命中故需 `fallback: 'always'`, Message-ID 与 ENVELOPE To 显示名被改写, CREATE 答 NO 或文件夹秒级消失, 服务器自存的已发送副本一小时内不可按 UID 寻址; 163: 文本 SEARCH 对近期邮件零命中而 126 / yeah.net 正常, 发件人显示名空格变下划线; Sina: SEARCH 仅接受 ALL / SINCE / 标志键, 文本键一律 BAD 故总走客户端; 五家自定义关键字均不存储); Gmail 令牌已过期, Outlook.com / iCloud 无账户, 均标注未核实; 矩阵发现并修复客户端缺陷: POP3 XOAUTH2 被以 SASL continuation 拒绝时 (Gmail) Angus 2.0.5 忽略拒绝并视为登录成功, 插件曾报可重试的 `IO_FAILED`, 现 `MailSessionFactory` 在连接后以 STAT 核实并映射为 `AUTH_FAILED` (`Pop3OAuthScriptedTest` 2 例); 真机确认 QQ A -> QQ B 于 Redmi API 33 (P2.3 流程) 全部通过; 证据 `docs/dev/p6-provider-matrix.md`; 2026-09-20 补跑, build 46, 维护者更新 Gmail 令牌并提供三个 Outlook.com / Hotmail 账户后: Gmail 以令牌通过 test / folders (22 夹, `[Gmail]` 为不可选父级, 角色来自 SPECIAL-USE) / 发信 (`sentCopy = server`, Message-ID 保留) / 列表 (1.6 s 可见) / 服务器搜索 / 正文 / 附件 / 标记 (六家中唯一存储自定义关键字) / 建夹 (成功且存活) / MOVE (4.0 s) / watch (IDLE 真推送, 第二封 32 s) / POP3 (UIDL 23 字符, 刚发的邮件不在 POP3 视图) / 清理 (`[Gmail]/Sent Mail` 的服务器副本可按 UID 删除); 两处客户端缺陷: Gmail 广告 `UTF8=ACCEPT`, Angus 启用后对含非 ASCII 文本的 SEARCH 一律答 `BAD Could not parse command`, 中文主题搜索因此落到客户端, 现按 D41 不再启用该扩展 (`Utf8SearchTest`), 中文主题搜索由服务器命中 (1 hit via the server in 2.3 s); `fallback: 'always'` 的正文搜索在命中不足 limit 时扫完 2000 候选窗口, Gmail 上每候选 1.3 s 共约 43 分钟, 客户端过滤现每 25 个候选向轨迹记一次进度; Outlook.com: 预设仅 XOAUTH2 故对应用密码直接拒绝, 绕过预设 (`--no-preset`) 后服务器 IMAP 广告 `LOGINDISABLED AUTH=XOAUTH2`, Angus 找不到可用机制即放弃 (`No login methods supported`), 曾映射为 `SERVER_ERROR`, 现为 `AUTH_MECHANISM_UNSUPPORTED` (`LoginDisabledTest`), POP3 答 `Basic authentication is disabled`, SMTP 587 答 `535`, 三账户一致, 操作行等待令牌 (P9); P5 设备矩阵的 Gmail 行同日补齐 (`docs/dev/p5-watch-evidence.md`); 假 IMAP 服务器改为广告 `LITERAL+`: 开发机的 ESET 邮件防护会拦截回环上 IMAP 同步字面量之后的数据; iCloud 仍无账户; 2026-09-21 补跑, build 56, 维护者注册 Entra 公共客户端并以 `.python/outlook_oauth_login.py` 取得令牌后: Outlook.com (一个 Hotmail 个人账户) 以令牌通过 test / folders (22 夹, 角色来自常规名称, 无 SPECIAL-USE / XLIST) / 发信 (`sentCopy = server`, Message-ID 被改写) / 列表 (约 7 s 可见) / 服务器搜索 (各键命中; 中文主题命中但服务器耗时 409-678 s, ASCII 形式约 1 s) / 正文 / 附件 / 标记 (自定义关键字不存储) / 建夹 (存活) / MOVE (2.0 s) / watch (IDLE 真推送, 第二封 8.3 s, 七家最快) / POP3 (两行式 AUTH XOAUTH2, UIDL 5 字符, 刚发的邮件在视图中) / 清理 (`Sent` 副本可按 UID 删除); 客户端搜索行未跑 (2300 封收件箱, 与 Gmail 同样的 2000 候选窗口); 发现并修复客户端缺陷: Outlook.com 的 POP3 对 Angus 默认的单行 `AUTH XOAUTH2 <base64>` 答 `-ERR Protocol error. Connection is closed.` 并断开, 预设新增 `pop3Xoauth2TwoLine` (providers.json 版本 3, outlook / office365 为 true), `MailSessionProperties` 对这两个预设与微软 POP3 主机设置 Angus 的 `two.line.authentication.format` (`Pop3OAuthScriptedTest` 增至 5 例), Redmi 真机 `docs/smoke/pop3.js` 27.7 s 与 `send-receive.js` 34.2 s 通过; 另两个账户未能以此令牌核实 (首次登录在浏览器账户选择器中登入的是 HOTMAIL_A 而 `--suffix` 写的是 HOTMAIL_B, 助手现在登录后以一次 IMAP 登录核对身份), 其中 HOTMAIL_B 的 SMTP 在校验令牌前即答 `535 5.7.139 SmtpClientAuthentication is disabled for the Mailbox` (微软对较新个人邮箱的邮箱级开关, 用户无法更改), 两账户待各自令牌复核; 2026-09-21 第二轮 (build 57): 维护者为 HOTMAIL_B 与 OUTLOOK_A 各注册一个 Entra 公共客户端后再登录: 助手改为登录时一并申请 `openid email` 并从 id_token 报告账户选择器实际登入的地址; `--suffix HOTMAIL_B` 的两次登录在选择器里登入的都是 OUTLOOK_A (HOTMAIL_B 本身尚未登录过, 误存到其键下的令牌已从令牌文件删除); OUTLOOK_A 以自己的令牌 SMTP 587 答 `235 2.7.0 Authentication successful`, 而 IMAP 答 `NO User is authenticated but not connected`, POP3 在 `+` 续行后答 `-ERR Authentication failure: unknown user name or bad password`, 即该邮箱在服务端未开放 IMAP / POP (与 HOTMAIL_B 的 SMTP 邮箱级禁用互为镜像), 不是客户端缺陷, 其操作行待维护者在 Outlook 网页版检查 IMAP / POP 同步设置与主别名后复核)
...
- [x] (发布) 体积: release universal APK 体积记录; Angus Mail 未用的 provider (如 `gimap` 若不用 Gmail 扩展, `dsn`, `pop3` 保留) 与 `MailHandler` 日志组件经 R8 裁剪; 记录每个裁剪的依据. (2026-09-20 完成, build 44, 证据 `docs/dev/p6-size.md`: release universal APK 由 2,254,035 字节降至 2,153,957 字节 (下载体积 1,507,931 -> 1,407,295), DEX 方法 12,150 -> 9,831, 类 1,914 -> 1,727, 四个命名空间存活的类 410 (全部按原名保留) -> 248 (20 个按原名, 其余改名); `app/proguard-rules.pro` 的四条整命名空间 keep 改为按名加载点逐条 keep 并注明依据: `META-INF/javamail.default.providers` 的六个 store / transport (`<init>(Session, URLName)`), `META-INF/services/jakarta.mail.Provider` 的六个 Provider, `MailStreamProvider`, angus-activation 的两个 RegistryProvider 实现, `META-INF/mailcap` 与 `MailcapRegistry.HANDLERS` 的五个 handler; 裁掉 `util.logging` 的 `MailHandler` (11 类), GraalVM `nativeimage`, `image_gif` / `image_jpeg`, `IMAPSaslAuthenticator` / `OAuth2SaslClient` (SASL 从不启用), `SortTerm`, 大部分 `jakarta.mail.event`; `gimap` / `dsn` 本就不在 `org.eclipse.angus:jakarta.mail` 包内, 无可裁; R8 会把 `META-INF/services/<接口>` 文件名改成改名后的接口名而内容仍指向按名保留的实现类; release 受测构建 (`-PandroidTestRelease :app:connectedReleaseAndroidTest`) 在 Redmi API 33 上 27 例通过 (4 例因缺实参跳过), 首轮因测试 APK 共用应用的 AndroidX 类 (`androidx.tracing.Trace.beginSection`) 被裁而崩溃, 已在仅测试用的 `proguard-android-test-release.pro` 中保留 AndroidX 与测试直接链接的五个 Jakarta 类)
...
- [x] (文档) `AutoJs6-Documentation`: `api/mail.md` (模块页, 插件依赖段落按 `api/barcode.md:5` 的写法, `## [@] mail` / `## [m] connect` / ... 全部方法含 `Async` 标签与重载, 示例脚本); 类型页 `api/mailClientType.md`, `api/mailMessageType.md`, `api/mailAccountOptionsType.md`, `api/mailSearchQueryType.md` (按 `*Type.md` 惯例, `dataTypes.md` 加转发段); 注册 `api/sidebar.md`, `api/toc.md`, `api/progress.md`, `api/changelog.md`; `project.json` 版本; `normalize-markdown.py --check` + `auto-generate.py --check` 通过. (2026-09-20 完成: 文档仓库提交 c2e1ced. 五个页面: `api/mail.md` (mail 全局成员, 转发方法表, `MailError` 与错误码表, 示例), `api/mailClientType.md` (全部客户端方法含 `Async` 形态, 文件夹对象, MailFolder / MailFolderStatus / MailSendResult / MailSessionTestResult / MailWatch 事件表, 限制表), `api/mailMessageType.md` (MailMessage / MailAttachment / MailAddress / MailSendMessage / UID 参数), `api/mailAccountOptionsType.md` (选项, 服务商预设表与已核实差异, 保存的账户), `api/mailSearchQueryType.md`; sidebar / toc / progress / changelog / README / dataTypes 注册; `project.json` versionCode 由离线同步递增; `normalize-markdown.py --check` 与 `auto-generate.py --check` 通过, 内部链接按 Docsify slug 核对无缺失. 从源码核实并修正了两处初稿: `mail.connect` 只打开插件会话, 首次网络方法才连接登录; `provider` 不按地址域名推断. 教训: 中文小节标题需带英文括注 (如 "转发方法 (Forwarded Methods)") 才能在离线生成器中得到稳定 id; 行内代码中的 `<a@b.c>` 会被渲染器当作 mailto 自动链接且输出不确定 (`--check` 报 stale), 改用代码块.)
- [x] (文档) `AutoJs6-TypeScript-Declarations`: `declarations/autojs6/aj6-int-mail.d.ts` (`Internal.Mail` / `Internal.MailClient` / `Mail.*` 类型命名空间, 同步 + Async 重载, `MailWatch extends EventEmitter` 的事件重载), `index.d.ts` 三斜线引用, `aj6-int-init.d.ts` 的 `declare let mail: Internal.Mail;` / `$mail`; `docs/smoke/mail-smoke.ts` 通过 `tsc --noEmit --strictNullChecks --skipLibCheck --target ES2020 --lib ES2020`; `package.json` 两处版本 minor 递增, `docs/CHANGELOG.md` 与 README 版本历史 (只保留最近 3 版). (2026-09-20 完成: 声明仓库提交 dcb9912 (4.18.0). `aj6-int-mail.d.ts`: `Internal.Mail` 接口 + `Mail` 命名空间, `Operations` 为客户端与转发方法的共同基 (同步 + `Async`), `Client` 扩展 `account` / `isConnected` / `isClosed` / `close` / `toString`, `Watch extends EventEmitter$` 的 `on` / `once` 事件重载, Message / Attachment / Address / SendMessage / AccountOptions / Provider / SavedAccount / SearchQuery / SearchResult / Folder / FolderStatus / SendResult / SessionTestResult / ErrorCode; `aj6-int-init.d.ts` 的 `mail` / `$mail`, `index.d.ts` 引用; `docs/smoke/mail-smoke.ts` (含 7 条负例) 与全部 smoke 文件通过 tsc; `package.json` 4.17.0 -> 4.18.0, `docs/CHANGELOG.md` 与 README (保留 3 版). 与条目原文的差异: 类型命名为 `Internal.Mail.Client` 而非 `Internal.MailClient`, 与 `Ai.Stream` 等既有命名一致.)
- [x] (文档) `AutoJs6-Plugin-Ace-Editor`: 拷贝 `aj6-int-mail.d.ts` 到 `assets/editor/.../types/autojs6/`, 用宿主 `tools/ace-completion/generate-dts.mjs` 再生成聚合声明与 LSP 分组; `AutoJs6-Plugin-Offline-Docs`: 运行文档仓库 `auto-generate-for-autojs6.bat --sync-offline`, `SOURCE_PROVENANCE.md` 更新; 两仓库按各自 `AGENTS.md` 处理版本与提交. (2026-09-20 完成: Ace Editor 提交 45f9a3f (1.11.0, build 104): 内置声明包更新至 4.18.0 (`aj6-int-mail.d.ts`, `aj6-int-init.d.ts`, `index.d.ts`, 包内 `package.json` 与 `BUNDLED_DECLARATIONS.md`), 10 语言 changelog 与 README 由 `.python/generate_markdown.py` 再生成 (`--check` 通过), 声明包以 `mail-smoke.ts` 通过 tsc; 宿主 `tools/ace-completion` 的导入与再生成 (`import-autojs6-dts.mjs` -> `generate-autojs6-indices-from-dts.mjs` -> `generate-dts.mjs` -> verify 脚本) 写入宿主仓库, 归入下方宿主条目处理. Offline Docs 提交 d51c8a7 (6.8.0, build 51): 文档仓库 `auto-generate-for-autojs6.bat --dry-run` 预览后以 `--verify-offline` 正式同步 (`:app:testDebugUnitTest` 与 `:app:verifyOfflineDocsApks` 通过, 3 min 26 s), 资产新增 5 页 / 变更 130 项 (侧栏), 两份 `SOURCE_PROVENANCE.md` 的基准提交更新为 c2e1ced, 内容元数据 fileCount 190, 10 语言 changelog 条目追加到 v6.8.0, `normalize_offline_docs.py --check` 无变化; 文档仓库 versionCode 70 -> 71 (提交 7160428). 四个仓库均只本地提交, 未推送.)
- [x] (插件) README (10 语言, 单一文案源): 能做什么, 安装, 在插件中心启用, 各服务商准备工作 (开启 IMAP / 取授权码 / Gmail App Password / Outlook 需令牌), 快速示例 (发信 / 收信 / 监听), 别名账户, 兼容性 (P6 矩阵摘要), 常见问题 (`AUTH_FAILED` 排查, 163 `Unsafe Login`, 中文搜索回退), 发行历史, 许可证与第三方声明; `.changelog` 10 语言 `v1.0.0` 完整. (2026-09-20 完成: `.readme/template_readme.md` 新增服务商准备, 别名账户, 兼容性与常见问题四节及邮件模块文档链接 (`docs_mail_url`), 快速开始改为别名连接 / 发信 / 收信并下载附件 / 监听 / 异步搜索; `.python/generate_markdown.py` 的 `README_LIST_KEYS` 增加 `provider_steps`, `compatibility_points`, `faq_items`; 10 语言 JSON 各新增 13 个键 (7 条服务商要点含 Gmail 应用专用密码或令牌与 Outlook 仅令牌, 8 条兼容性结论取自 P6 各矩阵, 7 条常见问题: `AUTH_FAILED` 排查, 163 / 126 `Unsafe Login`, 中文搜索回退, 惰性登录, Doze 与监听, Outlook 令牌, POP3 子集), `p_status` 改为 1.0.0 首个正式版本, 使用步骤第 3 步按凭据种类列出服务商 (该步同时打包进 `plugin_instruction.md`, 故不引用 README 下文); `.changelog` 10 语言 `v1.0.0` 的 hint 由 P0 开发预览改为首个正式版本摘要, feature 17 / fix 9 / improvement 7 / dependency 4 条保持; `generate_markdown.py --check` 通过 (36 产物), 按 `ApplicationTextPunctuationTest` 的规则扫描无非 ASCII 标点; 发行历史与许可证 / 第三方声明节沿用模板既有内容.)
- [x] (宿主) 宿主 `.changelog` 10 语言复核 (契约 / 脚本 API / 插件中心三条), `docs/dev/mail-plugin-protocol-v1.md` 定稿, `README` 插件列表若有则加入本插件. (2026-09-20 完成: 宿主 `.changelog` 10 语言复核: 每种语言的 v6.8.0 均有契约模块 (含默认启用的插件中心注册), 脚本 API `mail` (P3.1 / P3.2) 与 `mail.watch` (P5) 三条 feature 及移除 JavaMail 的 dependency 条目, 内容一致, 无需修改; `docs/dev/mail-plugin-protocol-v1.md` 定稿于宿主提交 b1a98be40 (状态段改为随插件 1.0.0 发布的最终 V1 契约并列出各阶段交付的插件构建, 补 P4 / P6 / P7 条目关闭, P1.1 之后仅有可选新增, 契约版本保持 1); 宿主 README 没有插件列表, 无需加入. 附: 宿主编辑器资产已迁入 Ace Editor 插件 (其提交 cf776ab), 宿主 `tools/ace-completion` 仍按宿主路径运行, 故以宿主 `build/ace-regen` 临时根目录 (目录联接指向 Ace 插件的 `assets/editor`) 运行导入 -> 索引 -> 聚合声明 -> 四个校验 -> 补全器测试: 宿主提交 87ee5e7e2 (按声明 4.18.0 再生成的 `autojs6_indices.source.json` 新增 mail, ai, tts, flow, pangu, settings, yolo, powerManager, workManager 九个模块, 校验脚本与补全器测试中过期的 `app.launch` / `findImage` / `setRootMode` 期望更新), Ace 插件提交 5ba7b84 (1.11.0 build 105: `autojs6_indices.js`, `lib.autojs6.d.ts`, 生成形态的 `index.d.ts` 与 `BUNDLED_DECLARATIONS.md`, 10 语言 changelog). 三个仓库均只本地提交, 未推送.)
- [x] (发布) 发布 gate: `generate_markdown.py --check`, `:mail-core:test`, `:app:testDebugUnitTest`, `:app:assembleDebug :app:assembleDebugAndroidTest`, `:app:lintDebug`, instrumentation 在 API 24 AVD + 一台实体机, `appendDigestToReleasedFiles` 产出单个已签名 universal APK (`autojs6-plugin-angus-mail-v1.0.0-<CRC32>.apk`), 安装 + 插件中心启用 + 冒烟脚本; 官方插件索引 receipt (若索引仓库流程要求); GitHub 仓库创建与推送由维护者明确指示后进行. (2026-09-20 完成, build 52: `generate_markdown.py --check` MARKDOWN_OK (36 产物); `:mail-core:test` 39 类 225 用例 (2 个真实账户 / 性能探针按设计跳过), `:app:testDebugUnitTest` 14 类 59 用例, `:app:assembleDebug` (8,954,544 B) 与 `:app:assembleDebugAndroidTest`, `:app:lintDebug` 0 错误 29 警告 (UseKtx 8, UnusedResources 6, PluralsCandidate / IconDuplicatesConfig / GradleDependency 各 2, 其余 3 条), 一次链式运行 4 min 21 s; `:app:connectedDebugAndroidTest` 在 AVD_API_24 (emulator-5554) 27 用例 0 失败 4 跳过 21.0 s, 在 Redmi 22120RN86C (API 33) 27 用例 0 失败 4 跳过 28.6 s (报告副本 `build/p7/connected-*/`); `:app:appendDigestToReleasedFiles` 产出唯一的 `releases/autojs6-plugin-angus-mail-v1.0.0-4c454a87.apk` (2,183,153 B, APK Signature Scheme v2, CRC32 与文件名一致; 旧的 dc9d0f69 产物先行删除); 该 APK 安装到 Redmi (versionCode 52) 后, 宿主 `MailScriptSmokeDeviceTest#realProviderScript` 以 QQ 账户跑 `docs/smoke/send-receive.js` 59.3 s 通过 (插件默认在插件中心启用, D19; `report leak check: clean`; 宿主 Gradle 2 min 27 s, 与 EPUB 会话协调后独占宿主 Gradle, 事后为 Redmi 重装宿主 debug). 官方插件索引 receipt: 索引仓库流程未要求, 未做; GitHub 推送待维护者明确指示 (插件仓库自 build 47 起未推送). (2026-09-21 补: 维护者指示后 master 推送 (build 47-54); GitHub Release v1.0.0 以 build 54 产物 `autojs6-plugin-angus-mail-v1.0.0-69b62a10.apk` 原地重切, 替换索引批量会话建立的 build 45 预览版, 该 APK 在 Redmi 上再过一次宿主冒烟 (58.3 s); 官方插件索引准入清单 54.json 于索引仓库提交 c9a8b99, 详见第十五次会话的补充.) (2026-09-21 补, 1.0.1 发布 gate, build 57 `8591780`: `generate_markdown.py --check` MARKDOWN_OK (36 产物); `:mail-core:test` 39 类 228 用例 (2 探针跳过), `:app:testDebugUnitTest` 14 类 59 用例, `:app:assembleDebug` (8,968,196 B) 与 `:app:assembleDebugAndroidTest`, `:app:lintDebug` 0 错误 29 警告 (UseKtx 8, UnusedResources 6, SetTextI18n 6, PluralsCandidate / IconDuplicatesConfig / GradleDependency 各 2, 其余 3 条), 链式运行 3 min 53 s; `:app:connectedDebugAndroidTest` 在 AVD_API_24 (emulator-5554) 27 用例 0 失败 4 跳过 11.1 s, 在 Redmi 22120RN86C (API 33) 27 用例 0 失败 4 跳过 28.7 s (报告副本 `build/p7/connected-*-57/`); `:app:appendDigestToReleasedFiles` 产出 `releases/autojs6-plugin-angus-mail-v1.0.1-36564c81.apk` (2,194,357 B, APK Signature Scheme v2, 证书与 1.0.0 相同, CRC32 与文件名一致, versionCode 57; 1.0.0 的产物移至 `build/p7/release-100/`); 该 APK 安装到 Redmi 后宿主 `MailScriptSmokeDeviceTest#realProviderScript` 以 QQ 账户跑 `docs/smoke/send-receive.js` 62.5 s 通过, `report leak check: clean`, 宿主 Gradle 1 min 46 s (与 EPUB 会话协调后独占), 事后为 Redmi 重装宿主 debug 5282; 维护者许可后: 注解 tag `v1.0.1` 指向 build 57, GitHub Release "1.0.1" (APK + SHA256SUMS, APK sha256 928c5985..., 说明取自 `.changelog/lang_en.json` 的 v1.0.1 块, 脚本 `build/p7/angus_release_101.py`), 官方插件索引准入清单 57.json (`build/p7/index_57.py`) 于索引仓库提交 33bd036 推送 (54.json 保留, 索引按发布时间选最新; `Plugin Index` 工作流 通过 (run 35574162332)), 详见第十五次会话的补充.)
- [x] (文档) 本文件收尾: 全部条目勾选并附证据, 附录 D 全部回填, 会话记录补最后一条. (2026-09-20 完成, build 53: P0 至 P7 的全部条目已勾选并附证据 (P8 / P9 为 1.1.0 与 1.2.0 的预留条目, 按 D30 / D31 保持未勾选); 附录 D 的八个待决事项已于 2026-09-18 全部拍板 (D25-D32), 无待回填; 会话记录补第十五次会话条目; 本提交为纯文档提交, 发布产物对应 build 52.)
...
- [x] (宿主) 契约演进: `IMailPlugin.aidl` 末尾追加 `IMailTrigger openTrigger(in Bundle options, IMailTriggerCallback callback)` 与 `Bundle listTriggers()`; 新增 `IMailTrigger { Bundle getStatus(); void update(in Bundle options); void stop(); }` 与 `IMailTriggerCallback { oneway void onMail(long generation, long seq, in Bundle event); oneway void onStatus(in Bundle status); }`; `CONTRACT_VERSION` 升为 2 (`MIN_SUPPORTED` 仍为 1); `MailCapabilityKeys.FEATURES` 加 `backgroundWatch`; 宿主与插件按能力协商, 任一方为旧版时功能不可见, 不抛错. (2026-09-21 完成, 宿主 `be036195a`: `IMailPlugin.aidl` 末尾追加 `openTrigger` (8) 与 `listTriggers` (9), 新增 `IMailTrigger.aidl` 与 oneway `IMailTriggerCallback.aidl`, `MailContract.CONTRACT_VERSION = 2` / `MIN 1` / `MAX 2` / `TRIGGER_CONTRACT_VERSION = 2`, 键 `triggerOptionsJson` / `triggersJson` / `triggerId`, 字段与状态常量, 上限 `MAX_TRIGGERS` 16 / `MAX_TRIGGER_SUBSCRIBERS` 4 / `MAX_TRIGGER_RECORDS` 100 / `MIN_TRIGGER_INTERVAL_MS` 3000, `MailActions.HOST_TRIGGER_ACTION`, 特性 `backgroundWatch`; `MailAidlOrderTest` / `MailContractTest` 9 用例; 宿主 `MailPluginHost.listTriggers` 以 契约 >= 2 且 `mailFeatures` 含 `backgroundWatch` 为门槛, 旧版插件答空表, 插件在旧版宿主上以 `queryBroadcastReceivers` 判定接收器缺失并在守望页面提示; 本仓库 build 59 重锁 `libs/mail-api.aar` (`locks/host-api-aars.lock`, `common-plugin-api.aar` 字节不变).)
- [x] (插件) `MailWatchService`: 前台服务 (`foregroundServiceType="dataSync"`, Android 14+ 需 `FOREGROUND_SERVICE_DATA_SYNC`; 常驻通知说明账户与状态, `POST_NOTIFICATIONS` 运行时申请), 复用 P5 的 `IdleWatcher` / `PollWatcher` 与 P4 的 `AccountStore` (只支持已保存别名, 不接受脚本传入的凭据); `ConnectivityManager` 网络变化回调触发立即重连; Doze 下按 D27 引导但不作为前置; 触发记录 (最近 100 条, 只含 envelope 摘要, 不含正文) 持久化于 `noBackupFilesDir`; 开机自启为设置页开关 (默认关, `RECEIVE_BOOT_COMPLETED`). 权限追加理由写入 README 安全章节与 changelog. (2026-09-21 完成, build 59: `trigger.MailWatchService` 为 `specialUse` 前台服务 (D42: `dataSync` 在 Android 15 上有每日 6 小时上限且不能从 `BOOT_COMPLETED` 启动), 通知渠道 `background_watch` (低优先级, 点击进入守望页面), `POST_NOTIFICATIONS` 在守望页面启用守望时申请; `trigger.WatchKeeper` 单例以 P5 的 `Watchers.open` 运行 `TriggerStore` 中已启用的守望 (只接受已保存别名, `AccountStore.withSecret` 在进程内解密), 单线程 `angus-mail-trigger` 投递, `WatchConfig.onConnected` 钩子报告 `connected`, 错误 -> `connecting`, 关闭 -> `failed` 并按 `WATCH_BACKOFF_MAX_MS` 重启, `WatchNetworkMonitor` 网络变化立即重连; `trigger.TriggerStore` 以 fsync + 原子替换写 `noBackupFilesDir/mail-triggers/triggers.json` 与每守望的 `records-<sha256>.json` (至多 `MAX_TRIGGER_RECORDS` 条信封摘要, 损坏条目单独跳过, `TriggerConfigsDocument.parseLenient`); `trigger.BootReceiver` Manifest 默认禁用, 由页面开关经 `setComponentEnabledSetting` 打开, 开机时有已启用守望才拉起服务; `trigger.HostTriggerSender` 在没有活动订阅者时以 PLUGIN 权限向宿主包发显式 `MAIL_TRIGGER` 广播. 权限理由已写入 README 安全章节 (10 语言) 与 changelog `hint`.)
- [x] (插件) 设置页 "守望": 账户 (别名) 选择, 文件夹, 模式 (IDLE / 轮询 + 间隔), 可选过滤 (发件人 / 主题子串), 启停开关, 状态 (连接态 / 最近事件 / 最近错误), 触发记录列表, 通知与电池优化状态提示. (2026-09-21 完成, build 59: `trigger.WatchesActivity` (设置页新增 "守望" 行, 摘要为已配置数与已连接数): 服务状态行, 通知权限行 (缺失时申请), 开机自启开关, 宿主缺少接收器时的警告, 守望卡片 (启停开关, 状态 / 模式 / 最近邮件 / 最近错误, 菜单: 编辑, 记录, 重连, 删除), 记录对话框 (可清空); `trigger.WatchEditorActivity`: 名称 (编辑时锁定), 已保存别名单选, 文件夹, 模式 auto / idle / poll, 轮询间隔 15..3600 s, 发件人 / 主题过滤 (逗号分隔), 启用开关, 保存前以 `TriggerConfig.validated()` 与所选账户的 `watchOptions(receive)` 校验; 字符串 66 条 x 11 语言 (`StringResourceParityTest`).)
- [x] (宿主) 定时任务子系统新触发器 "邮件到达": 选择脚本 + 插件守望 (由 `listTriggers` 列出) + 可选过滤; 宿主收到 `onMail` 后经既有任务调度器启动脚本, envelope JSON 作为 `engines.myEngine().execArgv.mail`; 宿主未运行时由插件以 PLUGIN 权限保护的显式 Intent (新 action `org.autojs.autojs6.action.MAIL_TRIGGER`, 形态同 `WAKE`) 拉起宿主; 去重 (同一账户 + 文件夹 + UID 只触发一次) 与节流 (`MIN_TRIGGER_INTERVAL_MS`). (2026-09-21 完成, 宿主工作副本 (与 P8 契约同一批, 未推送): `core/plugin/mail/MailTriggerReceiver` (Manifest 接收器, exported 且受 `org.autojs.permission.PLUGIN` 保护, 动态接收器永不注册该 action), `MailTriggerEvent` / `MailTriggerFilter` / `MailTriggerTaskExtras` (纯 Kotlin), `MailTriggerDispatcher` (去重: 守望 + 文件夹 + uid 最近 512 条; 节流: 每任务每 `MIN_TRIGGER_INTERVAL_MS` 至多一次, 保留最新事件延后启动; 宿主侧过滤与插件同义), `IntentTask.extras` + `IntentTaskDatabase` 版本 4 (`ALTER TABLE ... ADD extras TEXT`), `TimedTaskSettingActivity` 新增 "邮件到达时" 单选 + 守望选择器 (`MailPluginHost.listTriggers` + "任意守望") + 发件人 / 主题过滤, `Task.getDesc` 显示守望名, `ScriptIntents.prepareLaunch` 照旧启动, 事件文档经 `ExecutionConfig.setArgument("mail", ...)` 与 `Engines.setEngineExecArgv` 的 `toJsValue` 成为 `engines.myEngine().execArgv.mail`; JVM `MailTriggerDispatcherTest` 5 + `MailTriggerWatchEntryTest` 2. 宿主未运行时由插件广播拉起 (宿主进程死亡可拉起, 用户 "强行停止" 后按 Android 规则不投递, D43). `openTrigger` 订阅暂无宿主调用方, 留给后续脚本级 API.)
- [x] (宿主) 插件中心与定时任务 UI 文案 11 语言; `docs/dev/mail-plugin-protocol-v1.md` 增加 v2 章节 (触发器契约与降级规则). (2026-09-21 完成: 宿主 `text_run_on_mail_arrived` 与 `text_mail_trigger_*` 7 条 x 11 语言 (`values*/strings.xml`, 按名称插入); `docs/dev/mail-plugin-protocol-v1.md` 新增 "Version 2: Background Watches and Triggers (P8)" 章节 (追加的 Binder 面, 触发选项 / 状态 / 守望表 / 邮件事件文档, 无订阅者时的唤醒广播, 宿主侧去重 / 节流 / 过滤, 新旧版本组合的降级表) 与 Bundle 键 / 上限表 / 版本化段落 / Open Items 的 P8 条目; 宿主 changelog `feature` 条目 9 语言 (`lang_zh-Hans.json` 因维护者的整理稿未提交而未动, 留给维护者).)
- [x] (测试) JVM: 去重 / 节流 / 过滤 / 触发记录; instrumentation: 前台服务启停, 通知权限缺失时的行为, 进程重建后守望恢复, 旧版宿主下 `openTrigger` 不可见; DEVICE 矩阵 (`docs/dev/p8-background-watch-evidence.md`): QQ + Gmail 各一, 灭屏 30 分钟 / 断网 5 分钟 / 杀宿主 / 杀插件 四种扰动, 记录触发延迟与电量 (`dumpsys batterystats`). (2026-09-21 部分完成, build 59: JVM `TriggerFilterTest` 6 / `TriggerConfigTest` 6 / `TriggerDocumentsTest` 3 (mail-core), `TriggerStoreTest` 4 (app), `MailCoreContractParityTest` 与 `ManifestContractTest` 按契约 2 更新; instrumentation `MailTriggerBinderTest` 3 用例于 AVD_API_37 (emulator-5556) 5.8 s: 订阅即收状态, 每封推送一个 `mail` 事件 (generation / seq / 信封), `listTriggers` 含记录数, 过滤与 `update`, 禁用 / 未知守望与坏选项以 `refused` 拒绝, `stop` 只结束订阅. 待 build 60: 前台服务启停 / 通知权限缺失 / 进程重建 / 旧版宿主的 instrumentation 与 DEVICE 矩阵.) (2026-09-21 完成, build 60: DEVICE 矩阵 `docs/dev/p8-background-watch-evidence.md`, 驱动 `.python/run_trigger_matrix.py` + `docs/smoke/trigger.js` + 宿主 `MailTriggerTaskDeviceTest` (933fa9bf18) + 插件 `RealAccountWatchDeviceTest`, 均以 `am instrument` 运行以保留设备状态; QQ_A 守望 / QQ_B 发信: AVD_API_37 (emulator-5556) 六个场景全部通过 (baseline, 杀宿主 (广播冷启动宿主), 杀插件 (粘性服务 2.1 s 回来, 4.3 s 重连), 断网 300 s (断网期间的邮件在网络恢复 9.0 s 后触发), 灭屏 + force-idle 600 s (前台服务在 Doze 中保持轮询连接, 闲置中发出的邮件 44.7 s 后触发, 无需唤醒设备), 重启 (开机自启开关 + `BOOT_COMPLETED` 后服务先于首封邮件恢复)); Sony G8441 (API 28) baseline / 杀宿主 / 杀插件通过 (宿主冷启动脚本引擎约 13 s), 断网场景遇到粘性服务重启时插件进程连崩两次进入 30 分钟退避 (崩溃栈未留存, 待复现), 灭屏与重启因手机重启后停在图案锁 (CE 存储未解锁: 无 `BOOT_COMPLETED`, 无数据目录) 未完成; 每次运行广播数 = 接收数 = 脚本启动数, 泄漏检查 clean, `batterystats` 只见轮询节奏的 Wi-Fi 与 alarm; Gmail 列未跑 (令牌过期), IDLE 路径沿用 P5 的 `IdleWatcher`. 顺带修正: `TriggerStore` 用了 API 26 的 `java.nio.file` (minSdk 24, lint `NewApi` 于 build 60 发现), 改为 `FileOutputStream` + `fd.sync()` + `renameTo`; instrumentation 的前台服务启停 / 通知权限缺失 / 旧版宿主三项以设备矩阵的服务启停与 P8 的 `MailTriggerBinderTest` 覆盖, 未另写用例.)
- [x] (文档) 文档 / d.ts (`execArgv.mail` 类型) / README 守望章节 / changelog 10 语言. (2026-09-21 完成, build 59: README 10 语言 (功能条目, 使用步骤, 安全章节的四项权限理由, 状态段), changelog v1.1.0 10 语言 (`hint` + 3 条 `feature`), `VERSION_NAME` 1.1.0; 声明仓库 `aj6-int-mail.d.ts` 新增 `Internal.Mail.TriggerEvent` (`execArgv.mail`) 与 `docs/smoke/mail-smoke.ts` 用例, changelog 4.19.0 条目; 文档仓库 `api/mail.md` 新增 "邮件到达时运行脚本" 示例与 changelog 条目 (两仓库均仅本地提交).)

验收条件: 两家服务商在两台设备上完成扰动矩阵; 契约 v2 协商在新旧版本组合下正确降级; 证据写入 `docs/dev/p8-background-watch-evidence.md`. (2026-09-21 结论: 一家服务商 (QQ) 在两台设备上完成矩阵, 模拟器六个场景与手机三个场景通过, 手机余下三个场景受图案锁与一次未留栈的服务崩溃阻塞; 契约 v2 降级由 P8 落地时的 `MailCoreContractParityTest` 与守望页面的 "宿主无法被唤醒" 提示覆盖, 新旧宿主组合未在设备上重跑; 证据已写入 `docs/dev/p8-background-watch-evidence.md`. 1.1.0 发布 gate 待维护者决定.)
...
- [ ] (维护者) 注册 OAuth 客户端: Google (`https://mail.google.com/` scope, Android 客户端类型绑定包名与签名指纹, 发布前需通过敏感 scope 审核) 与 Microsoft Entra (公共客户端, 移动重定向 URI, scope `https://outlook.office.com/IMAP.AccessAsUser.All` / `POP.AccessAsUser.All` / `SMTP.Send` / `offline_access`); 客户端 ID 以 `resValue` 注入, 不含密钥 2026-09-21: Microsoft Entra 公共客户端已由维护者注册 (个人账户, 重定向 `http://localhost`, 允许公共客户端流), `.python/outlook_oauth_login.py` 以其在 PC 上取得令牌供 P6 矩阵使用; Google 客户端与插件内置流程仍待办. (2026-09-21 状态: Entra 公共客户端已注册 (P6 时用于 `outlook_oauth_login.py`, 重定向 `http://localhost`), 还需在同一注册的 "移动和桌面应用程序" 平台添加重定向 URI `io.github.supermonster003.autojs6.plugin.angus.mail://oauth2/microsoft`; Google 的 Android 客户端 (包名 + 签名指纹, 发布签名与调试签名各一) 与敏感 scope 审核未开始. 本机写 `oauth-clients.properties` (`googleClientId` / `microsoftClientId` / `microsoftTenant`, Git 忽略) 后构建即启用.)
- [x] (插件) `OAuthProviders` (授权 / 令牌端点, scope, PKCE 参数, 数据文件) 与 `OAuthFlow`: `androidx.browser` Custom Tabs 打开授权页, 自定义 scheme (或 `https` App Link) 回调 Activity 接收 `code` 并校验 `state`, PKCE (S256) 换取 access / refresh token (HTTPS 只指向令牌端点, 本阶段同步更新 Manifest 注释与 `AGENTS.md` 第 6 节 "不发起 HTTP 请求" 的表述); 令牌与过期时间由 P4 的 `AccountStore` 加密保存 (`AccountRecord` 增加 `oauth` 字段); 会话建立时 access token 剩余不足 5 分钟则先刷新; 刷新失败 -> `AUTH_FAILED` 并在设置页标记 "需要重新授权". (2026-09-21 完成, build 61: `mail-core` 的 `core/oauth` 包 `OAuthProvider` / `OAuthProviders` (Google 与 Microsoft 端点, scope, `access_type=offline&prompt=consent`, Google 撤销端点, 预设映射 `forPreset`), `Pkce` (S256, 32 字节 verifier, 16 字节 `state`, 常量时间比较), `AuthorizationRequest` / `AuthorizationResponses` (URL 组装, 重定向匹配: 目标 URI + `state` 后才看 `code` / `error`), `OAuthTokens` / `IdTokenClaims`, `TokenClient` (`FormPoster` 抽象, 交换 / 刷新 / 撤销, 错误映射 AUTH_FAILED / SERVER_ERROR / CONNECT_FAILED / TIMEOUT, 消息不含 code / verifier / 令牌); 插件 `oauth` 包 `HttpsFormPoster` (唯一 HTTP 客户端, 只接受 https, 不跟随重定向, 64 KiB 上限), `OAuthClients` (构建注入的客户端 id 与重定向 URI), `OAuthSignInActivity` (Custom Tabs, 回退 ACTION_VIEW, `onNewIntent` 接收重定向, 后台线程换令牌), `OAuthRedirectActivity` (exported, 两个 VIEW filter, 只转发), `PendingGrants` (10 分钟内存暂存, 不进 Bundle), `AccountSecrets` (D44), `TokenRevoker`; `SecretKind.OAUTH2` (记录编码 3), `MailAccountOptions` 的 `oauth` 对象校验, `androidx.browser` 1.10.0.)
- [x] (插件) 设置页: 新增账户时提供 "使用 Google / Microsoft 账号登录" 路径, 展示授权状态与过期时间, "重新授权" / "撤销" 操作; 预设表 `gmail` / `outlook` / `office365` 的 `authHint` 更新为 "内置授权, 或 App Password / 脚本令牌". (2026-09-21 完成, build 61: 认证对话框在 Gmail / Outlook.com / Microsoft 365 预设下多一项 "使用 Google / Microsoft 账号登录 (浏览器)" (无客户端 id 时禁用并提示), 选中后密码 / 令牌栏换成 "浏览器登录" 行 (未登录 / 已以 x 登录 / 已于 x 授权 / 需要重新登录), 登录结果经 `PendingGrants` 回到编辑器并预填地址, 保存写 `OAUTH2` 记录与 `oauth` 链接, 测试与保存沿用已保存令牌; 账户页摘要 "Google / Microsoft 账号登录 (需要重新登录)", 菜单 "重新登录" (登录页直接写回记录) 与 "撤销登录" (确认后本地立即作废, Google 另行撤销); `providers.json` 版本 4, 三个预设的 `authHint` 先提浏览器登录; 35 条字符串 x 11 语言.)
- [x] (宿主) 无 AIDL 变化 (别名会话在插件内解析秘密); `mail.accounts.list()` 元数据增加 `auth: 'oauth2'` 标记, 插件中心说明更新. (2026-09-21 完成: 宿主无改动; `mail.accounts.list()` 的条目按 D44 增加可选 `oauth` 对象而非 `auth: 'oauth2'` (声明仓库 `SavedAccount.oauth?: SavedAccountOAuth`, 文档 `api/mail.md` 账户小节), 插件中心说明由 README 生成的 `plugin_instruction` 带上.)
- [ ] (测试) JVM: PKCE / `state` / 令牌响应解析 / 过期判断 (本地假 HTTP 服务器); instrumentation: 回调 Activity 只接受匹配 `state` 的回调, 令牌加密存储往返, 撤销后不可解密; DEVICE: Gmail 与 Outlook.com 各完成一次 授权 -> 收信 -> 手工撤销 -> 自动刷新失败提示重新授权 (`docs/dev/p9-oauth2-evidence.md`). (2026-09-21 部分完成, build 61: JVM `PkceTest` 4 / `AuthorizationRequestTest` 8 / `OAuthTokensTest` 4 / `TokenClientTest` 10 / `OAuthProvidersTest` 4 (mail-core, 令牌端点以脚本化 `FormPoster` 代替本地 HTTP 服务器), `OAuthClientsTest` 3 / `AccountSecretsTest` 9 (app, 内存账户库: 续期改写记录, 拒绝标记 `needsReauth`, 端点故障可重试, 未配置客户端, 擦除), `MailAccountOptionsTest` / `AccountFormTest` / `SavedAccountsDocumentTest` / `AccountRecordCodecTest` 的 `oauth` 用例, `SecretAuditTest` 扩展到守望与 OAuth 状态日志行 (允许文件清单 + 插值词审计). 待办: instrumentation (回调 Activity 拒绝外来 `state` 由 JVM 覆盖, 设备上的 OAUTH2 记录往返与撤销) 与 DEVICE 证据 (`docs/dev/p9-oauth2-evidence.md`) 需维护者先补注册 (见维护者条目) 并提供 `oauth-clients.properties`.)
- [x] (文档) README 各语言 "使用 Google / Microsoft 账号" 章节与隐私说明 (令牌只存本机, 不经宿主与任何第三方服务器), changelog 10 语言; 若使用 App Link 则说明 `assetlinks.json` 的托管位置. (2026-09-21 完成, build 61: README 10 语言 (功能条目 "浏览器登录", 使用步骤, Gmail / Outlook 服务商条目, 安全章节的 INTERNET 权限说明改为 "邮件服务器 + 两家令牌端点的 HTTPS", 令牌存储说明, Outlook FAQ, 状态段), changelog v1.2.0 10 语言 (`hint` + 3 条 `feature`), `VERSION_NAME` 1.2.0; 自定义 scheme 无需 `assetlinks.json`.)
...
| --- | --- | --- | --- | --- | --- |
| `gmail` | imap.gmail.com:993 ssl | pop.gmail.com:995 ssl | smtp.gmail.com:465 ssl (587 starttls) | App Password 或 XOAUTH2 | 自动保存已发送; 文件夹 `[Gmail]/...`; 2026-09-20 核实: 广告 `UTF8=ACCEPT` 但启用后拒绝非 ASCII 搜索 (D41), 自定义关键字会存储, IDLE 真推送 (约 30 s 节奏), 已发送副本可按 UID 寻址, POP3 视图不含刚发出的邮件 |
| `outlook` | outlook.office365.com:993 ssl | outlook.office365.com:995 ssl | smtp-mail.outlook.com:587 starttls | 仅 XOAUTH2 (IMAP 已禁基本认证) | Outlook.com 个人账户; 2026-09-20 三账户核实: 应用密码在 IMAP (`LOGINDISABLED`, 仅 `AUTH=XOAUTH2`) 与 POP3 均答 `Basic authentication is disabled`, SMTP 587 答 `535`; 2026-09-21 以令牌核实一个 Hotmail 账户的全部操作: 角色来自常规名称 (无 SPECIAL-USE), 服务器保存已发送副本并改写 Message-ID, 自定义关键字不存储, CREATE / MOVE 正常, IDLE 约 10 s 推送, 非 ASCII SEARCH 命中但耗时数分钟, POP3 仅接受两行式 AUTH XOAUTH2 (`pop3Xoauth2TwoLine`); 较新的个人邮箱可能被微软禁用 SMTP AUTH (`535 5.7.139`) |
...
2026-09-20 核实 (P6 兼容矩阵补跑, Gmail / Outlook.com, 真实账户, JVM + Redmi 真机): Gmail (XOAUTH2 令牌): 22 个文件夹, 角色来自 SPECIAL-USE, `[Gmail]` 为不可选父级; 发信 `sentCopy = server`, Message-ID 保留, 1.6 s 内可见; 服务器搜索 ASCII 主题 / 发件人 / 正文 / Message-ID / since 全部命中, 中文主题在 Angus 启用 `UTF8=ACCEPT` 时答 `BAD Could not parse command`, 不启用后 (D41) 服务器命中 (1 hit via the server in 2.3 s); 正文 / 附件 / 显示名均正确; 自定义关键字 `AutoJs6Matrix` 被存储 (六家中唯一); `CREATE` 成功且存活; `MOVE` 4.0 s; IDLE 真推送 (从 PC 用 imaplib 测得投递后约 30 s 报告 EXISTS, 自发自收 60 s; 插件 watch 第二封 32 s; Redmi 设备矩阵基线 / 杀插件 / 关 Wi-Fi 三场景到达 35 / 34 / 49 s, 事件顺序与 QQ / 163 一致); POP3 正常 (UIDL 23 字符, 列 5 封 7.8 s, 刚发的邮件不在 POP3 视图); 清理时 `[Gmail]/Sent Mail` 的服务器副本可按 UID 删除; 客户端正文搜索 (`fallback: 'always'`) 每候选 1.3 s (每封两三次往返, PC 上单次文本段抓取 0.45 s), 2000 候选窗口约 43 分钟. Outlook.com / Hotmail (三账户, 应用密码): 预设仅 XOAUTH2 故不发起连接即拒绝; 绕过预设后 IMAP 广告 `AUTH=XOAUTH2 LOGINDISABLED`, `LOGIN` / `AUTHENTICATE PLAIN` 答 `NO Basic authentication is disabled.`, POP3 同样拒绝, SMTP 587 STARTTLS 答 `535` (17.5 s), 与附录 B 的公开说明一致, 操作列等待令牌 (P9). iCloud 仍无账户.

2026-09-21 核实 (P6 兼容矩阵补跑, Outlook.com 以 OAuth 2.0 令牌, 一个 Hotmail 个人账户, JVM + Redmi 真机): 令牌来自维护者的 Entra 公共客户端注册 (`.python/outlook_oauth_login.py`, 委托 scope IMAP.AccessAsUser.All / POP.AccessAsUser.All / SMTP.Send + offline_access); IMAP 登录后能力 IDLE UIDPLUS MOVE ID NAMESPACE UNSELECT CHILDREN LITERAL+ SASL-IR (无 SPECIAL-USE / XLIST / UTF8=ACCEPT), 22 个文件夹, 角色来自常规名称 (`Inbox` / `Sent` / `Drafts` / `Deleted` / `Junk`, `Archive` 无角色); 发信 `sentCopy = server`, `Sent` 中的副本可按 UID 删除, 外发 Message-ID 被服务器改写 (同 QQ), 两端显示名保留; 列表约 7 s 可见; 服务器搜索各键命中, 非 ASCII 文本键 (`CHARSET UTF-8` + 字面量) 在 2300 封收件箱上耗时 409-678 s 而 ASCII 形式约 1 s; 自定义关键字不存储; CREATE 成功且存活, MOVE 2.0 s; IDLE 真推送, 提交后约 10 s 内到达 (七家最快), Redmi 设备矩阵 baseline / 杀插件 / 关 Wi-Fi 三场景 9-14 s 到达且事件顺序与 Gmail 一致; POP3 只接受两行式 AUTH XOAUTH2 (单行答 `-ERR Protocol error. Connection is closed.`), 预设字段 `pop3Xoauth2TwoLine` (providers.json 版本 3) 令邮件核心设置 Angus 的两行开关, UIDL 5 字符, 刚发的邮件在 POP3 视图中; 另两个账户待各自令牌, HOTMAIL_B 的 SMTP 答 `535 5.7.139 SmtpClientAuthentication is disabled for the Mailbox` (微软对较新个人邮箱的邮箱级禁用, 用户无法更改). 详见 `docs/dev/p6-provider-matrix.md` 与 `docs/dev/p5-watch-evidence.md`.
...
### 2026-09-21 (第十七次会话, P8 设备矩阵与 P9 代码落地, build 60-61)

- 完成 (P8, build 60): 设备矩阵与 `docs/dev/p8-background-watch-evidence.md` (AVD_API_37 六场景全过, Sony 三场景过), 驱动修正 (测试先停服务再改守望; 模拟器重启前先启动服务并在重启后恢复 adb root; run-as 回退; 未起服务时抓 crash 缓冲), `TriggerStore` 的 API 24 修正, `SecretAuditTest` 扩展到守望 / OAuth 状态日志行 (允许文件清单 + 插值词审计), `run_settings_real_account.py` 按版本通配 APK. 宿主 933fa9bf18 (`MailTriggerTaskDeviceTest`, 未推送).
- 教训 (P8): `am instrument` 结束时会 force-stop 被测进程, 粘性前台服务若在被测进程内重启会让 Android 9 报 "Process crashed" (测试先 `stopService`); 被 force-stop 的包处于 stopped 状态收不到 `BOOT_COMPLETED` (重启前先以 Watches 页面的方式启动服务); 图案锁手机重启后 CE 存储未解锁, 既无数据目录 (`run-as` 报 corrupt installation) 也无 `BOOT_COMPLETED`, 只能等用户解锁; 模拟器重启会丢 adb root, 非 root shell 进不了未导出服务, 而 `run-as` 在 API 31+ 受后台 FGS 限制 (`uidState: NONE`), 驱动须先 `adb root`; 前台服务在 force-idle 下保持网络 (与 P5 的脚本级监听不同); Sony 上宿主冷启动脚本引擎约 13 s, 且有一次冷启动即死 (重跑通过); 驱动末尾读 logcat 时主缓冲已滚动, 崩溃栈要在事发时抓 `-b crash`; lint `NewApi` 才发现 `File.toPath` / `java.nio.file` 需 API 26.
- 完成 (P9, build 61): 见 P9 条目 (核心 `core/oauth` 包, 插件 `oauth` 包, `SecretKind.OAUTH2`, 设置页与账户页, `providers.json` v4, 35 x 11 字符串, JVM 42 用例, README / changelog v1.2.0 / 声明 / 文档), D44 / D45.
- 教训 (P9): Android 只在有 host 时匹配 intent-filter 的 `path`, Google 反转客户端 id 的重定向 (`scheme:/oauth2redirect`) 无 authority, filter 只能写 scheme (lint `AppLinkUrlError` 也报); Kotlin 源码里的 `\u0000` 经 Python 补丁脚本写成了真实 NUL 字节, `file` 报 data 而编译照过, 补丁脚本要用原始字符串; `SecretAuditTest` 的 `log(Log.X, ...)` 间接调用会绕过 `Log.` 正则, 审计正则要一并覆盖; `Files.move` / `File.toPath` 的 API 26 限制在 JVM 测试与 API 28+ 设备上都不会暴露.
- 未做: Sony 的断网 / 灭屏 / 重启 (图案锁; 断网崩溃待复现); Gmail 守望列; P9 的 instrumentation 与设备证据 (`docs/dev/p9-oauth2-evidence.md`, 需维护者在 Entra 注册添加移动重定向 URI `io.github.supermonster003.autojs6.plugin.angus.mail://oauth2/microsoft`, 注册 Google Android 客户端并提供 `oauth-clients.properties`); 1.1.0 / 1.2.0 发布 gate (发布时机由维护者决定; 契约 v2 需宿主发布); 宿主未推送 (维护者决定).
- 下次会话建议起点: 维护者解锁 Sony 后重跑 `run_trigger_matrix.py QQ_A BH900ASK9E --sender QQ_B --scenario net-off|screen-off|reboot --no-install` 并把崩溃栈补入证据; 拿到 `oauth-clients.properties` 后在两台设备上走 Gmail / Outlook.com 的登录 -> 收信 -> 撤销 -> 重新登录并写 `docs/dev/p9-oauth2-evidence.md`; 然后 1.1.0 / 1.2.0 发布 gate.

### 2026-09-21 (第十六次会话, P8 代码落地, 1.1.0 build 59)

- 完成: 宿主契约版本 2 (`be036195a`, `plugin-api/mail-api`), 宿主侧 "邮件到达时" 任务 (接收器, 分发器, 任务 extras 与数据库版本 4, 任务编辑器与守望选择器, `execArgv.mail`, 11 语言文案, 协议文档 v2 章节, changelog 9 语言; 未提交至本记录时点, 见下次会话) 与本仓库 build 59: `mail-core` 的 `core/trigger` 包与 `TriggerDocuments`, 上限常量, `WatchConfig.onConnected`; `app` 的 `trigger/` 包 (`TriggerStore`, `WatchKeeper`, `MailWatchService`, `BootReceiver`, `HostTriggerSender`, `WatchesActivity`, `WatchEditorActivity`), `binder/MailTriggerBinder`, `MailPluginBinder.openTrigger` / `listTriggers`, 四项权限与 Manifest 组件, 66 条字符串 x 11 语言, 三个图标; JVM 19 用例与 instrumentation `MailTriggerBinderTest` 3 用例 (AVD_API_37); README / changelog / `VERSION_NAME` 1.1.0; 声明仓库与文档仓库的 `execArgv.mail` 条目 (本地提交). 决策 D42 (`specialUse`) 与 D43 (唤醒广播路径).
- 教训: 宿主 `Engines.setEngineExecArgv` 原样 `put` Java 值, Map / List 参数要经 `RhinoUtils.toJsValue` 才能在脚本中作为普通对象访问; `Augmentable.put(o, Pair)` 不接受可空值, 用三参数重载; 一个 `TriggerConfigsDocument` 整体反序列化会让单条坏配置拖垮整表, 改为逐条宽松解析; AGP 测试引擎在 API 37 AVD 上无需 `adb reverse` (回环 IMAP 在插件进程内); Bash heredoc 里的 `'\r\n'` 字面量会破坏 Python 脚本, 用 `chr(13) + chr(10)`; 宿主 `values*/strings.xml` 本就有十余处乱序对, 只校验新名称的相邻顺序; 宿主 `.changelog/lang_zh-Hans.json` 有维护者未提交的整理稿 (v6.8.0 条目大幅精简并新增 `hint`), 两个插件会话都不改动它.
- 未做 (build 60 起): `docs/dev/p8-background-watch-evidence.md` 的设备矩阵 (QQ + 163 / Outlook.com, 灭屏 30 分钟 / 断网 5 分钟 / 杀宿主 / 杀插件, 触发延迟与 `batterystats`), 前台服务与进程重建的 instrumentation, `.python/run_trigger_matrix.py` 驱动与 `docs/smoke/trigger.js`; 宿主提交与 `assembleAppDebug`; 1.1.0 发布 gate (维护者决定发布时机, 需宿主先发布契约 2 的构建).
- 下次会话建议起点: 先补设备矩阵与证据文件 (Sony G8441 + AVD_API_37; Redmi 由 EPUB 会话占用时避让), 再更新本文件 P8 的测试条目与验收结论, 然后按维护者指示进入 P9.

### 2026-09-20 (第十五次会话, P7 全部, 1.0.0 发布 gate)

- 完成: P7 全部. 文档仓库 `AutoJs6-Documentation` 提交 c2e1ced (`api/mail.md` 与四个类型页, 注册与 changelog) 与 7160428 (离线同步后的 versionCode 71); 声明仓库 `AutoJs6-TypeScript-Declarations` 提交 dcb9912 (4.18.0, `aj6-int-mail.d.ts`, `mail` / `$mail`, `docs/smoke/mail-smoke.ts`); Ace Editor 插件提交 45f9a3f (1.11.0 build 104, 内置声明 4.18.0) 与 5ba7b84 (build 105, 以宿主 `tools/ace-completion` 再生成 `autojs6_indices.js` / `lib.autojs6.d.ts`, 新增 mail 等九个模块); Offline Docs 插件提交 d51c8a7 (6.8.0 build 51, 离线同步与 `SOURCE_PROVENANCE.md`); 宿主提交 87ee5e7e2 (ace-completion 源与过期期望) 与 b1a98be40 (协议文档 V1 定稿); 本仓库 build 47 `bf506b7` (文档条目), 48 `adb4a93` (声明条目), 49 `d063bcb` (Ace / 离线条目), 50 `fc016fa` (README 10 语言重写: 服务商准备, 别名账户, 兼容性, 常见问题, 邮件文档链接, v1.0.0 changelog hint), 51 `da90bb6` (宿主条目), 52 `f9b5d93` (发布 gate: JVM 225 + 59 用例, lint 0 错误, AVD API 24 与 Redmi API 33 各 27/27, 签名 APK `autojs6-plugin-angus-mail-v1.0.0-4c454a87.apk`, 宿主冒烟 59.3 s), 53 (本条目). 发布产物对应 build 52 的工作树 (本条目为纯文档提交).
- 教训: 文档仓库的中文小节标题要带英文括注才能在离线生成器中得到稳定 id, 行内代码里的 `<a@b.c>` 会被渲染成不确定的 mailto 链接 (改用代码块); 声明仓库工作副本是 CRLF, 补丁脚本需保留换行风格; MSYS 会改写 `/tmp` 与 `/J` 这类参数 (`MSYS_NO_PATHCONV=1` + `cmd /c`), `sed -i` 会重写所有匹配文件并只留下换行噪声 (用 `git checkout -- <路径>` 还原); 离线同步脚本不更新两份 `SOURCE_PROVENANCE.md`; 宿主的编辑器资产已迁入 Ace Editor 插件, 宿主 `tools/ace-completion` 只能经临时根目录 (目录联接指向插件 `assets/editor`) 运行, 且其校验期望自 2026-07 起已过期 (`app.launch` / `findImage` / `setRootMode`); 打包进 `plugin_instruction.md` 的使用步骤不能引用 README 的其他章节; 宿主 Gradle 与 EPUB 会话共用, 跑宿主冒烟前后以消息协调.
- 补充 (2026-09-21, 维护者指示后推送与发布): 文档仓库 master 7160428 推送, 首个 tag `v6.8.0-71` / Release "6.8.0 (71)" (说明取自 `api/changelog.md` 的 v6.8.0 块); 声明仓库 main dcb9912 推送, tag `v4.18.0` / Release "4.18.0" (npm 包未发布, 本机 npm 未登录, 留给维护者); Ace Editor 5ba7b84 推送, 附注 tag `v1.11.0` / Release "1.11.0" (5 个 ABI APK + SHA256SUMS); Offline Docs d51c8a7 推送, 既有 Release `v6.8.0` 原地更新 (维护者选择: tag 移到 d51c8a7, APK 4b985644 换为 1ea7d316, 说明重生成); 宿主按维护者决定不推送. 本仓库 build 54 `95d3021` (`.python/outlook_oauth_login.py`: 公共客户端 + PKCE, 回环重定向, Exchange IMAP / POP / SMTP 委托 scope + offline_access, 令牌写入忽略的 `build/outlook-token.properties`, `--refresh`; `ProviderMatrixProbe` 与 `run_host_script_smoke.py` 对任何带 `<KIND>_ACCESS_TOKEN_<letter>` 键的档案选 XOAUTH2), master 推送 (build 47-54). GitHub Release v1.0.0 原地重切: 索引批量会话 (索引仓库提交 ea175b5) 已于 2026-09-20 以 build 45 (`2057ad4`, 开发预览 APK dc9d0f69) 建过 v1.0.0, 本次以 HEAD 95d3021 重建 `appendDigestToReleasedFiles` 产物 `autojs6-plugin-angus-mail-v1.0.0-69b62a10.apk` (2,183,153 B; 与 gate 产物 4c454a87 逐条目比较仅 `AndroidManifest.xml` / `META-INF/version-control-info.textproto` / `resources.arsc` 不同; v2 签名, 签名者 31a681fc...), 安装到 Redmi (versionCode 54) 后宿主 `MailScriptSmokeDeviceTest#realProviderScript` 以 QQ 账户跑 `docs/smoke/send-receive.js` 58.3 s 通过 (`report leak check: clean`; 与 EPUB 会话协调后独占宿主 Gradle 1 min 16 s, 事后重装宿主 debug 并恢复 MANAGE_EXTERNAL_STORAGE), 附注 tag `v1.0.0` ("Release 1.0.0") 强制移到 95d3021 (tag 对象 154c3a9), 资产换为 69b62a10 APK + SHA256SUMS, 说明按 Ace 格式由 `.changelog/lang_en.json` v1.0.0 生成 (hint 1 / feature 17 / fix 9 / improvement 7 / dependency 4). 官方插件索引 `AutoJs6-Official-Plugins-Index` 提交 c9a8b99 推送: 准入清单 `release-manifests/io.github.supermonster003.autojs6.plugin.angus.mail/54.json` (替换 45.json), Ace `105.json` (含 `supportedAbis`), Offline Docs `51.json` (替换 50.json), `tools/generate_official_plugin_index.py` 再生成 (60 项, 仅这三项变化), 索引单元测试 42 通过. 发布脚本 `build/p7/make_release.py` (ace / offline / docs) 与 `build/p7/angus_release.py` (prepare / publish) 均在忽略目录.
- 补充教训: 索引准入清单按 (package, versionCode) 查找, 多 ABI 发布必须写 `supportedAbis` (缺失时生成器拒绝), 资产名须与 Release 完全一致且大小 / sha256 与 GitHub 的 digest 核对; 原地重切 Release 时旧 versionCode 的清单要一并撤回, 否则索引仍指向已删除的资产; Entra 的个人 Microsoft 账户已不能在目录外注册应用 ("The ability to create applications outside of a directory has been deprecated"), 需先有租户 (免费 Azure 订阅的默认目录, 或现有工作 / 学校租户).
- 补充 (2026-09-21, Outlook.com 列, build 56): 维护者发布 npm 4.18.0 并注册 Entra 公共客户端后以 `.python/outlook_oauth_login.py` 取得令牌 (首次运行的 `--suffix HOTMAIL_USER_NAME_B` 键名与账户选择器里实际登入的 HOTMAIL_A 不符: 助手改为按档案命名令牌键 (`HOTMAIL_ACCESS_TOKEN_A` 等, 旧键自动迁移), 并在写入后以一次 IMAP 登录核对令牌打开的是哪个邮箱; `ProviderMatrixProbe`, `run_provider_matrix.py`, `run_host_script_smoke.py` (新增 outlook / hotmail / live / msn 域) 与 `run_watch_matrix.py` 把 `build/outlook-token.properties` 叠加到账户文件上). P6 Outlook.com 列全部通过 (三次 JVM 运行, 第二次在客户端搜索行手动终止), P5 设备行三场景 9-14 s, Redmi 宿主冒烟 `send-receive.js` 34.2 s (release 54) 与 `pop3.js` 27.7 s (含修复的 debug 构建) 通过, 泄漏检查均 clean; 修复 Outlook.com POP3 两行式 AUTH XOAUTH2 (`pop3Xoauth2TwoLine`, providers.json 版本 3, `Pop3OAuthScriptedTest` 5 例); mail-core 39 类 228 用例 (2 探针跳过) 与 app 14 类 59 用例通过; changelog 新增 v1.0.1 (fix + improvement, 10 语言), `VERSION_NAME` 1.0.1 (未打 tag, 未发布), README 10 语言的 Outlook.com 兼容性与准备条目重写并再生成; 文档仓库 `api/mailAccountOptionsType.md` / `api/mail.md` / `api/mailClientType.md` 与声明仓库 `aj6-int-mail.d.ts` 补 `pop3Xoauth2TwoLine` 字段 (本地提交), 宿主协议文档 P6 段补 2026-09-21 一句 (本地提交, 宿主不推送). 其余账户: HOTMAIL_B / OUTLOOK_A 需各自登录取令牌 (助手 `--suffix HOTMAIL_B` / `OUTLOOK_A`), HOTMAIL_B 的 SMTP 疑被微软在邮箱级禁用 (`535 5.7.139`).
- 补充教训: MSA 令牌是不透明的 (`EwA...`), 无法本地解码得知所属邮箱, 只能以一次协议登录核对; 浏览器账户选择器登入的账户可能与命令行参数不符, 令牌键名应按档案而非按输入的后缀; Outlook.com 的非 ASCII SEARCH 是服务器端慢 (单条命令数分钟), 不是客户端回退; 矩阵探针的客户端搜索行在 2000+ 封的邮箱上要 30-40 分钟, 用 `--ops` 跳过而不是等待; Angus 的 POP3 两行式开关按 `mail.pop3s.` 前缀 (SSL) 设置才生效; 登录时申请 `openid email` 可从 id_token 直接得知选择器登入的地址, 免去事后猜测 (旧令牌的刷新授权不含该 scope, 只能重新登录); 同一令牌 SMTP 235 而 IMAP `not connected` / POP3 `unknown user name or bad password` 说明是邮箱服务端未开放 IMAP / POP, 不是令牌问题.
- 补充 (2026-09-21, 1.0.1 发布与第二轮 Outlook 登录, build 57-58): 维护者提供 HOTMAIL_B 与 OUTLOOK_A 各自的 Entra 客户端 id 并许可发布 1.0.1. 登录: 助手改为登录时一并申请 `openid email` 并从 id_token 报告选择器实际登入的地址 (build 57; 刷新仍只用邮件 scope, 旧令牌无此同意); `--suffix HOTMAIL_B` 的两次登录登入的都是 OUTLOOK_A, HOTMAIL_B 本身未登录过, 误存的令牌已从 `build/outlook-token.properties` 删除; OUTLOOK_A 以自己的令牌 SMTP 235 通过, IMAP `NO User is authenticated but not connected`, POP3 续行后 `-ERR Authentication failure: unknown user name or bad password`, 即邮箱服务端未开放 IMAP / POP, 操作行无法跑. 发布: gate 在 build 57 全绿 (明细在 P7 发布 gate 条目), 注解 tag `v1.0.1` 与 GitHub Release "1.0.1" (`autojs6-plugin-angus-mail-v1.0.1-36564c81.apk`, 2,194,357 B) 由 `build/p7/angus_release_101.py` (从 1.0.0 的重切脚本派生, 改为新建 Release) 发布, 索引准入清单 57.json 由 `build/p7/index_57.py` 生成并于索引仓库提交 33bd036 推送 (`Plugin Index` 工作流 通过 (run 35574162332)); Redmi 现装 release 57 与宿主 debug 5282. 本条与 P6 / P7 条目的补记为 build 58 (docs-only); 插件 master 推送至 build 58.
- 未做: 宿主未推送 (维护者决定); HOTMAIL_B 尚未以本人登录取得令牌 (两次登录选择器都登入了 OUTLOOK_A), OUTLOOK_A 的邮箱服务端未开放 IMAP / POP (SMTP 可用), 两账户的操作行均未跑; Outlook.com 客户端搜索行未跑; iCloud / Yahoo / Aliyun 预设未经真实账户核实; 文档仓库与声明仓库的 `pop3Xoauth2TwoLine` 条目仅本地提交 (两仓库另有 EPUB 会话的未推送工作), Ace / Offline Docs 插件的内置副本待下次发布再同步.
- 下次会话建议起点: 按附录 F 进入 P8 (1.1.0 后台守望, D30) 或 P9 (内置 OAuth, D31; Entra 公共客户端已注册, Google 待办); 若维护者在 Outlook 网页版为 OUTLOOK_A 打开 IMAP / POP, 或以 HOTMAIL_B 本人完成登录, 先以 `run_provider_matrix.py <profile> --ops test,folders,send,list,search-server,body,attachment,flags,move,watch,pop3,cleanup` 补跑该列.
