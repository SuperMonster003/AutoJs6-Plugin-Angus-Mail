...
- 补充教训: MSA 令牌是不透明的 (`EwA...`), 无法本地解码得知所属邮箱, 只能以一次协议登录核对; 浏览器账户选择器登入的账户可能与命令行参数不符, 令牌键名应按档案而非按输入的后缀; Outlook.com 的非 ASCII SEARCH 是服务器端慢 (单条命令数分钟), 不是客户端回退; 矩阵探针的客户端搜索行在 2000+ 封的邮箱上要 30-40 分钟, 用 `--ops` 跳过而不是等待; Angus 的 POP3 两行式开关按 `mail.pop3s.` 前缀 (SSL) 设置才生效.
- 未做: 宿主未推送 (维护者决定); 1.0.1 (build 56, 含 POP3 两行式修复) 未打 tag / 未发布, 是否发布由维护者决定; HOTMAIL_B / OUTLOOK_A 两账户待各自令牌复核; Outlook.com 客户端搜索行未跑; iCloud / Yahoo / Aliyun 预设未经真实账户核实; 文档仓库与声明仓库的 `pop3Xoauth2TwoLine` 条目仅本地提交 (两仓库另有 EPUB 会话的未推送工作), Ace / Offline Docs 插件的内置副本待下次发布再同步.
- 下次会话建议起点: 维护者决定是否发布 1.0.1 (发布 gate 同 P7: 单元测试, lint, 两台 instrumentation, `appendDigestToReleasedFiles`, 宿主冒烟, GitHub Release 与索引准入清单 56.json); 之后按附录 F 进入 P8 (1.1.0 后台守望, D30) 或 P9 (内置 OAuth, D31; Entra 公共客户端已注册, Google 待办).
