...
| `outlook` | outlook.office365.com:993 ssl | outlook.office365.com:995 ssl | smtp-mail.outlook.com:587 starttls | 仅 XOAUTH2 (IMAP 已禁基本认证) | Outlook.com 个人账户; 2026-09-20 三账户核实: 应用密码在 IMAP (`LOGINDISABLED`, 仅 `AUTH=XOAUTH2`) 与 POP3 均答 `Basic authentication is disabled`, SMTP 587 答 `535`, 操作列等待令牌 (P9) |
...
- 未做: 宿主未推送 (维护者决定); npm `@sm003/autojs6-dts` 4.18.0 未发布; Outlook.com 操作行待维护者完成 Entra 应用注册并运行令牌助手 (P9 前置); iCloud / Yahoo / Aliyun 预设未经真实账户核实; 二次冒烟只跑了 Redmi 一台 (AVD 与 Redmi 的 instrumentation 已覆盖 API 24 / 33).
- 下次会话建议起点: 维护者提供 Entra 应用的 client id 后运行 `py .python/outlook_oauth_login.py --client-id <id> --suffix A`, 再跑 `.python/run_provider_matrix.py OUTLOOK_A` 补 P6 Outlook.com 列与 P5 设备行; 之后按附录 F 进入 P8 (1.1.0 后台守望, D30) 或 P9 (内置 OAuth, D31).
