...
Evidence for roadmap P8 collected on 2026-09-21 in this repository (Gradle 9.5.0, AGP 9.3.2,
Kotlin 2.3.20, JDK 21, Windows 11) and in the host repository (AutoJs6 6.8.0, build 5282). The
plugin side is build 59 (`996e07f`, the P8 feature) plus the driver, test and store fixes of build
60 (this document); the host side is `6ad80448f` (contract version 2, the task, the receiver and
the dispatcher) plus `933fa9bf18` (the device test that installs the task). The host debug APK used
here was assembled at 18:50 from `04c55ac78a`, which contains both.
...
| BH900ASK9E | Sony G8441 | 9 (API 28) | baseline, kill-host, kill-plugin; the net-off run hit a service crash loop (below), the screen-off and reboot runs are blocked: the phone rebooted into its pattern lock (credential-encrypted storage: no `BOOT_COMPLETED` for a non-direct-boot-aware receiver, no data directory, `run-as` answers "corrupt installation") and only the maintainer can unlock it |
...
| net-off | BH900ASK9E | #1 | None | - | - | - | 0 | no script: the script for #1 |
| net-off | BH900ASK9E | #2 | None | - | - | - | 0 | sent while offline; ran - after the network came back; no script: the script for #2 |
| net-off | BH900ASK9E | #3 | None | - | - | - | 0 | no script: the script for #3 |
...
- **net-off / BH900ASK9E**: broadcasts 0, receiver events 0, script launches 0, final state None (None), service in the foreground at the end no, plugin threads -, secret in logcat no, address in logcat no.
...
- **net-off**: the keeper reported `connecting` during the outage (reconnects with backoff), the
  network change listener reconnected it 9.0 s after the network came back and the mail sent
  during the outage was delivered then; the mail after the outage took the usual polling latency.
- **reboot** (emulator): with the boot switch on, `BOOT_COMPLETED` reached the enabled
  ...
  unlocks it (the Sony above), which is the platform's rule for credential-encrypted storage and
  cannot be helped by the plugin; the README says that watches resume after the device is unlocked.
- **screen-off** (emulator): with the screen off and the device forced into deep idle for ten
  ...
  cadence and the alarm wake lock only.
- **Sony net-off** (build 59): after the instrumentation had rewritten the watch, the system's
  restart of the sticky service crashed the plugin process twice within a second of its start
  ("Showing crash dialog", then the 30-minute restart backoff), so the run collected nothing; by
  the time the driver read the log at the end of the run the phone's main buffer had rotated past
  the `AndroidRuntime` stack, and the reboot scenario afterwards cleared the crash buffer. The Sony
  kill-plugin run three minutes earlier restarted the same service cleanly. To be reproduced once
  the phone is unlocked; the driver now snapshots `logcat -b crash` right after a service start
...
- Sony G8441: net-off (crash loop above), screen-off and reboot (the pattern lock); the 30 min
  screen-off row on a phone therefore rests on the P5 matrix (Doze freezes the network, the watch
  reconnects when the device wakes) and on the emulator row here.
