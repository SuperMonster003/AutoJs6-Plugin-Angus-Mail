 ...
 * Wi-Fi off and on, Wi-Fi to cellular). A watch blocked in IDLE on a connection the old network
 * took with it would otherwise notice only at its next renew; [Watcher.reconnect] drops the
 * connection so the loop reconnects at once with the backoff reset. Registered with the system
 * only while at least one watch runs (`ACCESS_NETWORK_STATE`, declared already). Failures to
 * register leave the watches to their own timers.
 */
internal class WatchNetworkMonitor(context: Context) {
    ...
    fun detach(watcher: Watcher) {
        synchronized(lock) {
            watchers -= watcher
            if (watchers.isEmpty() && registered) {
                registered = false
                current = null
